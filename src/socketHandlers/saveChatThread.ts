...
export const saveChatThread = async (userId: mongoose.Types.ObjectId, shopId: string, userQuery: string, gptResponse: string) => {  
    ...
    try {
        ...
        console.log("recommendation count:",jsonObject.recommendation_count)
    //     if (Array.isArray(jsonObject.recommendation_count)) {
    //       parsedRecommendations = jsonObject.recommendation_count;
    //       jsonObject.recommendation_count.forEach((recommendation: any, index: number) => {
